      ...
      <div class="flex flex-wrap mb-24 -mx-3">
        <div class="w-full pr-2 lg:w-1/4 lg:block">
          <div class="p-4 mb-5 bg-white border border-gray-200 dark:border-gray-900 dark:bg-gray-900">
            ...
            {{-- <div class="text-red-500">
              Featured: {{ var_export($featured, true) }} (type: {{ gettype($featured) }})
            </div>
            --}}
        ...
        @endforeach
          ...
          <label for="{{$brand->slug}}" class="flex items-center dark:text-gray-300">
            <input type="checkbox" wire:model.live="selected_brands" value="{{$brand->id}}" class="w-4 h-4 mr-2"
            id="{{$brand->slug}}">
        ...
        @endforeach
            ...
            <ul>
              ...
              <li class="mb-4">
                <label for="on_sale" class="flex items-center dark:text-gray-300">
                  <input type="checkbox" class="w-4 h-4 mr-2" value="1" wire:model.live="on_sale" id="on_sale">
          ...
          <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
            ...
            <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
            <div>
              <div class="mb-4 text-lg font-bold dark:text-gray-400"> {{Number::currency($price_range, 'EUR')}} </div>
              <input type="range" wire:model.live="price_range"
                class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer" max="50000" value="10000"
                step="100">
              <div class="flex justify-between ">
                <span class="inline-block text-lg font-bold text-blue-400 "> {{Number::currency(0, 'EUR')}} </span>
                <span class="inline-block text-lg font-bold text-blue-400 "> {{Number::currency(50000, 'EUR')}} </span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{$product->id}}">
          ...
          <div class="p-3 ">
            ...
            <p class="text-lg ">
            <span class="text-green-600 dark:text-green-600"> {{Number::currency($product->price, 'EUR')}}
            </span>
            </p>
          </div>
